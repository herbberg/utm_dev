    ...
    switch (state)
    {
      case 0UL:
      {
        if (n == "number" && ns.empty ())
        {
          if (start)
          {
            this->::xml_schema::complex_content::context_.top ().parser_ = this->number_;
          ...
          else
          {
            if (this->number_)
            {
              unsigned int tmp (this->number_->post_unsigned_int ());
              this->number (tmp);
            }
...
namespace tmxsd
{
                             ...
                             const ::xml_schema::ro_string& s)
  {
    if (n == "bin_id" && ns.empty ())
    {
      ...
      {
        this->bin_id_->pre ();
        this->bin_id_->_pre_impl ();
        this->bin_id_->_characters (s);
        this->bin_id_->_post_impl ();
        unsigned int tmp (this->bin_id_->post_id ());
        this->bin_id (tmp);
      }
